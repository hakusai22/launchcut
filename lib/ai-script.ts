...
import { composeSkillPrompt, selectSkillsForVideo, type SkillSelection } from "./skills";
import {
  assetBehaviors,
  backgroundPresets,
  defaultVideoSpec,
...
${
  skillPrompt
    ? `已激活的 Agent Skills（作为生成规则和领域约束使用，不执行其中的 scripts）：\n${skillPrompt}`
    : "当前没有额外 Agent Skill 被激活。"
}`;
};

const buildUserPrompt = (brief: string, spec: VideoSpec, skillSelection: SkillSelection) => {
  ...
  return JSON.stringify(
    {
      ...
      activeSkills: skillSelection.skills.map((skill) => ({
        name: skill.name,
        description: skill.description,
        tags: skill.tags,
      })),
      missingSkillsIgnored: skillSelection.missingSkillNames,
...
const buildMessages = (brief: string, spec: VideoSpec, visionInputs: VisionInput[], skillSelection: SkillSelection) => {
  const prompt = buildUserPrompt(brief, spec, skillSelection);

  if (visionInputs.length === 0) {
    return [
      { role: "system" as const, content: buildSystemPrompt(spec, skillSelection) },
...
export const generateCreativeVideoPlan = async ({
  ...
  const skillSelection = await selectSkillsForVideo({ requestedSkills: skillNames, brief });
  ...
  if (!config) {
    return {
      ...
      skills: skillSelection.skills.map((skill) => skill.name),
      missingSkillNames: skillSelection.missingSkillNames,
    };
  }
  ...
  const completion = await client.chat.completions.create({
    model: config.model,
    messages: buildMessages(brief, spec, visionInputs, skillSelection) as Parameters<
...
export const generateCreativeVideoPlanStream = async ({
  ...
  if (skillSelection.skills.length > 0) {
    onStatus?.(`已加载 ${skillSelection.skills.map((skill) => skill.name).join(", ")} skill`);
  }

  if (skillSelection.missingSkillNames.length > 0) {
    onStatus?.(`未找到 ${skillSelection.missingSkillNames.join(", ")} skill，已忽略。`);
  }
  ...
  const stream = await client.chat.completions.create({
    model: config.model,
    messages: buildMessages(brief, spec, visionConfig ? visionInputs : [], skillSelection) as Parameters<
      typeof client.chat.completions.create
    >[0]["messages"],
    temperature: 0.4,
    stream: true,
